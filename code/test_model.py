...
def test_model(X_test, y_test, dt, optical_flows_test):
    ...
    with tf.device('/gpu:0'):
        ...
        model_out = encoder_decoder_pass(X, delta_t, optical_flows) * 255.0
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        print('Checking model performance on test data now. Process starting.') # for jupyter notebook
        print()
        model_logger.info('Checking model performance on test data now. Process starting.')
        
        test_size = X_test.shape[0]
        feed = {X: X_test, delta_t : np.ones((test_size, 1)) * float(dt), 
                    optical_flows : optical_flows_test}
...
if __name__ == '__main__':
    ...
    data = dataset_loader(delta_t = 20.0, k = 10, offset = 15) # make sure pickled data is loaded. should use data identical/comparable to training data. 
